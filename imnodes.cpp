...
namespace IMNODES_NAMESPACE
{
    namespace
    {
        // [SECTION] bezier curve helpers  ベジェ曲線ヘルパー
        ...
        // Calculates the closest point along each bezier curve segment.  各ベジェ曲線セグメントに沿って最も近い点を計算
        ...
        {
            // Trivial case: rectangle contains an endpoint  自明なケース: 長方形に端点が含まれています
            if (rect.Contains(p1) || rect.Contains(p2))
            {
                return true;
            }

            // Flip rectangle if necessary  必要に応じて長方形を反転します
            ...
            // Trivial case: line segment lies to one particular side of rectangle  自明なケース: 線分が長方形の特定の辺にある
            ...
            // At least one corner of rectangle lies on a different side of line segment  長方形の少なくとも 1 つの角が線分の異なる側にあります
        ...
        inline bool RectangleOverlapsLink(
            ...
            const ImNodesAttributeType start_type)
        {
            // First level: simple rejection test via rectangle overlap:  レベル1: 長方形の重なりによる単純な拒否テスト:
            ...
            {
                // First, check if either one or both endpoinds are trivially contained  まず、一方または両方のエンドポイントが自明に含まれているかどうかを確認します。
                // in the rectangle  長方形の中で
                ...
                // Second level of refinement: do a more expensive test against the  レベル2: より高価なテストを実行します。
                // link
        ...
        // [SECTION] coordinate space conversion helpers  座標空間変換ヘルパー
        ...
        // [SECTION] draw list helper  描画リストヘルパー
        ...
        void ImDrawListGrowChannels(ImDrawList* draw_list, const int num_channels)
        {
            ...
            for (int i = old_channel_count; i < requested_channel_count; ++i)
            {
                ...
                // If we're inside the old capacity region of the array, we need to reuse the existing  アレイの古い容量領域内にいる場合は、既存の容量領域を再利用する必要がある
                // memory of the command and index buffers.  コマンドバッファとインデックスバッファのメモリ
                if (i < old_channel_capacity)
                {
                    channel._CmdBuffer.resize(0);
                    channel._IdxBuffer.resize(0);
                }
                // Else, we need to construct new draw channels.  それ以外の場合は、新しい描画チャネルを構築する必要がある
                else
                {
        ...
        void DrawListAppendClickInteractionChannel()
        {
            ...
            // 注: この関数を EndNodeEditor の外で使用しないでください。すべてのノードが完了する前にこれを使用すると、
            // added will screw up the node draw order.
            //追加するとノードの描画順序がめちゃくちゃになります
            ImDrawListGrowChannels(GImNodes->CanvasDrawList, 1);
        }

        int DrawListSubmissionIdxToBackgroundChannelIdx(const int submission_idx)
        {
            // NOTE: the first channel is the canvas background, i.e. the grid  注: 最初のチャンネルはキャンバスの背景、つまりグリッドです
        ...
        void DrawListSortChannelsByDepth(const ImVector<int>& node_idx_depth_order)
        {
            ...
            while (node_idx_depth_order[start_idx] == GImNodes->NodeIdxSubmissionOrder[start_idx])
            {
                if (--start_idx == 0)
                {
                    // early out if submission order and depth order are the same  送信順序と深さ順序が同じ場合は早期アウト
                    return;
                }
            }

            // TODO: this is an O(N^2) algorithm. It might be worthwhile revisiting this to see if the time
            // complexity can be reduced.  複雑さを軽減

            for (int depth_idx = start_idx; depth_idx > 0; --depth_idx)
            {
                const int node_idx = node_idx_depth_order[depth_idx];

                // Find the current index of the node_idx in the submission order array  送信順序配列内のnode_idxの現在のインデックスを見つける
        ...
        // [SECTION] ui state logic  UI状態ロジック
        ...
        bool MouseInCanvas()
        {
            // This flag should be true either when hovering or clicking something in the canvas.  このフラグは、キャンバス内で何かをホバーするかクリックするときに true になる必要があり
        ...
        void BeginNodeSelection(ImNodesEditorContext& editor, const int node_idx)
        {
            ...
            if (!editor.SelectedNodeIndices.contains(node_idx))
            {
                ...
                // Ensure that individually selected nodes get rendered on top  個別に選択したノードが最前面にレンダリングされるようにする
            ...
            // Deselect a previously-selected node  以前に選択したノードの選択を解除します
            else if (GImNodes->MultipleSelectModifier)
            {
                ...
                // Don't allow dragging after deselecting  選択解除後のドラッグを許可しない
...
namespace IMNODES_NAMESPACE
{
    ...
    void BeginNodeEditor()
    {
        ...
        // Reset state from previous pass  前回のパスからの状態をリセット
        ...
        ImGui::BeginGroup();
        {
            ...
            // NOTE: we have to fetch the canvas draw list *after* we call  注: 呼び出した*後*、キャンバス描画リストをフェッチする必要があります。
            // BeginChild(), otherwise the ImGui UI elements are going to be  BeginChild()、それ以外の場合、ImGui UI 要素は次のようになる
            // rendered into the parent window draw list.  親ウィンドウの描画リストにレンダリングされる
    ...
    void EndNodeEditor()
    {
        ...
        // ImGui インタラクションは UI の残りの部分とのインタラクションをブロックするため、最初に検出する
    ...
    {
        // Remember to call BeginNodeEditor before calling BeginNode  BeginNodeを呼び出す前に、必ず BeginNodeEditor を呼び出すこと
        IM_ASSERT(GImNodes->CurrentScope == ImNodesScope_Editor);
        GImNodes->CurrentScope = ImNodesScope_Node;
    ...
    void EndNode()
    {
        ...
        // The node's rectangle depends on the ImGui UI group size.  ノードの四角形は、ImGui UI グループのサイズによって異なる
        ImGui::EndGroup();
        ImGui::PopID();
