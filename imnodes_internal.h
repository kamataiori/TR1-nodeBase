...
// The object T must have the following interface:  オブジェクト T には次のインターフェイスが必要
//
// struct T
// {
//     T();
...
// Emulates std::optional<int> using the sentinel value `INVALID_INDEX`.  センチネル値 `INVALID_INDEX` を使用して std::optional<int> をエミュレート
...
{
    int    Id;
    ImVec2 Origin; // The node origin is in editor space  ノードの原点はエディター空間内にある
...
struct ImPinData
{
    ...
    ImVec2               Pos; // screen-space coordinates  スクリーン空間の座標
    int                  Flags;
...
struct ImClickInteractionState
{
    ...
    struct
    {
        ImRect Rect; // Coordinates in grid space  グリッド空間内の座標
    } BoxSelector;
...
// [SECTION] global and editor context structs  グローバルおよびエディターのコンテキスト構造体
...
struct ImNodesEditorContext
{
    ...
    // ui related fields  UI関連分野
    ImVec2 Panning;
    ImVec2 AutoPanningDelta;
    // Minimum and maximum extents of all content in grid space. Valid after final  グリッド空間内のすべてのコンテンツの最小および最大範囲
    ...
    // Relative origins of selected nodes for snapping of dragged nodes  ドラッグされたノードのスナップ用に選択されたノードの相対原点
    ImVector<ImVec2> SelectedNodeOffsets;
    // Offset of the primary node origin relative to the mouse cursor.  マウス カーソルを基準としたプライマリ ノードの原点のオフセット。
    ImVec2 PrimaryNodeOffset;

    ImClickInteractionState ClickInteraction;

    // Mini-map state set by MiniMap()  MiniMap() によって設定されるミニマップの状態
    ...
    // Mini-map state set during EndNodeEditor() call  EndNodeEditor() 呼び出し中に設定されるミニマップの状態
...
struct ImNodesContext
{
    ...
    // Canvas draw list and helper state  キャンバス描画リストとヘルパー状態
    ...
    // Canvas extents  キャンバス範囲
    ImVec2 CanvasOriginScreenSpace;
    ImRect CanvasRectScreenSpace;

    // Debug helpers  デバッグヘルパー
    ...
    // Event helper state  イベントヘルパーの状態
    // TODO: this should be a part of a state machine, and not a member of the global struct.  TODO: これは、グローバル構造体のメンバーではなく、ステート マシンの一部である必要がある
    // Unclear what parts of the code this relates to.  これがコードのどの部分に関係するかは不明
    int ImNodesUIState;
...
namespace IMNODES_NAMESPACE
{
    ...
    {
        // No editor context was set! Did you forget to call ImNodes::CreateContext()?  エディターのコンテキストが設定されていません! ImNodes::CreateContext() を呼び出すのを忘れていませんか?
        IM_ASSERT(GImNodes->EditorCtx != NULL);
        return *GImNodes->EditorCtx;
    }

    // [SECTION] ObjectPool implementation  オブジェクトプールの実装
    ...
    inline void ObjectPoolUpdate(ImObjectPool<ImNodeData>& nodes)
    {
        for (int i = 0; i < nodes.InUse.size(); ++i)
        {
            ...
            else
            {
                ...
                {
                    // Remove node idx form depth stack the first time we detect that this idx slot is  この IDX スロットが初めて検出されたときに、深度スタックからノード IDX を削除
                    // unused  未使用
    ...
    static inline int ObjectPoolFindOrCreateIndex(ImObjectPool<T>& objects, const int id)
    {
        ...
        // Construct new object  新しいオブジェクトを構築する
        if (index == -1)
        {
        ...
        // Flag it as used  使用済みとしてフラグを立てる
    ...
    inline int ObjectPoolFindOrCreateIndex(ImObjectPool<ImNodeData>& nodes, const int node_id)
    {
        ...
        // Construct new node  新しいノードを構築する
        if (node_idx == -1)
        {
        ...
        // Flag node as used  ノードを使用中としてフラグを立てる
...
} // namespace IMNODES_NAMESPACE  ネームスペース IMNODES_NAMESPACE
